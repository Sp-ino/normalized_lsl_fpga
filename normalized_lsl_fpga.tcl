...
proc checkRequiredFiles { origin_dir} {
 ...
 "[file normalize "$origin_dir/src/delay_for_structures.vhd"]"\
 "[file normalize "$origin_dir/src/update_block.vhd"]"\
 ...
 "[file normalize "$origin_dir/src/tb/update_block_tb.vhd"]"\
 "[file normalize "$origin_dir/src/wave_config/tb_behav_update_block.wcfg"]"\
  ]
  foreach ifile $files {
...
set_property -name "webtalk.xsim_launch_sim" -value "739" -objects $obj
...
set file "$origin_dir/src/delay_for_structures.vhd"
set file [file normalize $file]
set file_obj [get_files -of_objects [get_filesets sources_1] [list "*$file"]]
set_property -name "file_type" -value "VHDL" -objects $file_obj

set file "$origin_dir/src/update_block.vhd"
set file [file normalize $file]
set file_obj [get_files -of_objects [get_filesets sources_1] [list "*$file"]]
set_property -name "file_type" -value "VHDL" -objects $file_obj
...
# Create 'test_update' fileset (if not found)
if {[string equal [get_filesets -quiet test_update] ""]} {
  create_fileset -simset test_update
}

# Set 'test_update' fileset object
set obj [get_filesets test_update]
set files [list \
 [file normalize "${origin_dir}/src/tb/update_block_tb.vhd"] \
 [file normalize "${origin_dir}/src/wave_config/tb_behav_update_block.wcfg"] \
]
...
# Set 'test_update' fileset file properties for remote files
set file "$origin_dir/src/tb/update_block_tb.vhd"
set file [file normalize $file]
set file_obj [get_files -of_objects [get_filesets test_update] [list "*$file"]]
set_property -name "file_type" -value "VHDL" -objects $file_obj


# Set 'test_update' fileset file properties for local files
# None

# Set 'test_update' fileset properties
set obj [get_filesets test_update]
...
# The design that will be created by this Tcl proc contains the following 
# module references:
# c_port_resizer, c_port_resizer, delay, delay, delay, delay, normalize, postmul_resizer, postmul_resizer_wideinput, postmul_resizer_wideinput
  ...
  connect_bd_net -net ck_1 [get_bd_ports ck] [get_bd_pins betae/CLK] [get_bd_pins delay_22/i_ck] [get_bd_pins delay_23/i_ck] [get_bd_pins delay_24/i_ck] [get_bd_pins delay_25/i_ck] [get_bd_pins e_minus_veb/CLK] [get_bd_pins veb/CLK] [get_bd_pins ys_plus_veb/CLK]
  connect_bd_net -net delay_22_o_data [get_bd_pins c_port_resizer_0/i_data] [get_bd_pins delay_22/o_data]
  connect_bd_net -net delay_23_o_data [get_bd_pins delay_23/o_data] [get_bd_pins v1_plus_betae/A]
  ...
  connect_bd_net -net v1_m_1 [get_bd_ports v1_m] [get_bd_pins delay_23/i_data] [get_bd_pins veb/A]
...
# delay, normalize
  ...
  # Create instance: minus_betageb, and set properties
  set minus_betageb [ create_bd_cell -type ip -vlnv xilinx.com:ip:dsp_macro:1.0 minus_betageb ]
  set_property -dict [ list \
   CONFIG.a_binarywidth {0} \
   CONFIG.a_width {18} \
   CONFIG.areg_3 {false} \
  ...
  # Create port connections
  connect_bd_net -net beta_m_1 [get_bd_ports beta_m] [get_bd_pins minus_betageb/A]
  connect_bd_net -net c_addsub_0_S [get_bd_ports gam_m1] [get_bd_pins c_addsub_0/S]
  connect_bd_net -net ck_1 [get_bd_ports ck] [get_bd_pins delay_1/i_ck] [get_bd_pins minus_betageb/CLK]
  connect_bd_net -net delay_1_o_data [get_bd_pins c_addsub_0/A] [get_bd_pins delay_1/o_data]
  connect_bd_net -net gam_m_1 [get_bd_ports gam_m] [get_bd_pins delay_1/i_data]
  connect_bd_net -net gam_minus_betageb_P [get_bd_pins minus_betageb/P] [get_bd_pins normalize_0/i_data]
  connect_bd_net -net geb_m_1 [get_bd_ports geb_m] [get_bd_pins minus_betageb/B]
  connect_bd_net -net normalize_0_o_data [get_bd_pins c_addsub_0/B] [get_bd_pins normalize_0/o_data]
